...
exp_name="countdown_greso"

# GRESO sampling params
greso_enable=true
greso_default_rollout_bsz=128  # b_r: number of prompts selected per greso rollout iteration
greso_p_easy=0.5              # retention probability per step for easy (all-correct) problems
greso_p_hard=0.5              # retention probability per step for hard (all-wrong) problems
greso_p_delta=0.01             # delta for probability adjustments
...
python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.rollout.sampling_method=greso \
    +greso.enable=${greso_enable} \
    +greso.default_rollout_bsz=${greso_default_rollout_bsz} \
    +greso.p_easy=${greso_p_easy} \
    +greso.p_hard=${greso_p_hard} \
    +greso.p_delta=${greso_p_delta} \
    +adv_predictor.log_level_every_problem=false \
