...
def compute_score(solution_str, ground_truth) -> float:
    retval = 0.0
    if solution_str is None or solution_str == "":
        print("DEBUG: Empty or None response")
    if THOUGHT_DELIMITER_END in solution_str:
        print("DEBUG: THOUGHT_DELIMITER_END found")
        model_solution = solution_str.split(THOUGHT_DELIMITER_END)[1]
    else:
        print("DEBUG: extracting solution from original response")
    ...
    if model_answer is None:
        print("DEBUG: No answer found")
        return retval
    
    if "\\boxed" in ground_truth:
        print("DEBUG: Ground truth is \\boxed{}")
        ground_truth = extract_answer(ground_truth)
    
    is_correct = grade_answer_mathd(model_answer, ground_truth) or grade_answer_sympy(model_answer, ground_truth)
    retval = 1.0 if is_correct else 0.0
    return retval
...
def extract_answer(passage: str) -> str:
    if "\\boxed" in passage:
        print("DEBUG: model answer is in \\boxed{}")
